...
$(document).ready(function(){
	...
	var bbTagsIntro ={};// BBCodeParser.defaultTags();
	
    addInlineTags(bbTagsIntro,function(){
	addInlineTags(bbTags,function(){
	    addBlockTags('',null,bbTags,function(){
		var parser = new BBCodeParser(bbTags);
		console.log(parser);
		var parserIntro = new BBCodeParser(bbTagsIntro);
		
		$('[data-entry="bbcode"]').keyup(function(e){
		    if($(e.target).attr('data-option') == 'intro')
			$('[data-output="bbcode"][data-entry-name="'+$(e.target).attr('name')+'"]').html(parserIntro.parseString(e.target.value))
		    else
			$('[data-output="bbcode"][data-entry-name="'+$(e.target).attr('name')+'"]').html(parser.parseString(e.target.value))
		})
	    })
	})	
    })
...
function addBlockTags(eltName,images,bbTags,next){
	...
	bbTags["fig"] = BBTag.createTag("fig", function (tag, content, attr) {
	    ...
	    if(images && images[link]){

		switch(images[link].ext){
		case 'svg': return "<figure><picture><source type=\"image/svg+xml\" srcset=\""+ '/images/blog/'+eltName+'/'+link+'.svg' +"\"><img src=\""+'/images/blog/'+eltName+'/'+link+'-medium.png'+"\" alt=\""+images[link].alt+"\"></picture><figcaption>" + caption + "</figcaption></figure>";
		    break;
